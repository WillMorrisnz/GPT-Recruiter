...
def evaluate_cv_against_job_description(cv, job_description, evaluation_categories):
  ...
  system_prompt = f"""As an extremely experienced recruiter with an exceptional knowledge of the tech industry, 
  ...
  Please think deeply about your reasoning before coming to a rating. 
  Format your response in raw JSON, providing a rating on a scale of 1-5 and a brief reasoning for each category. 
...
def craft_cover_letter_for_role(cv, job_description):
  system_prompt = """When provided with a job description and an applicant's CV, craft a succinct and professional cover letter tailored to the job opening,
  ensuring the letter does not surpass 250 words and is structured into three key paragraphs:
...
def evaluate_seek_job_listings(search_term, num_jobs, job_searh_radius_km, save_to_db=True, create_cover_letter=False):
  job_data_dict = scrape_service.scrape_seek_job_data(search_term, num_jobs, job_searh_radius_km)
    ...
    if not database_service.job_id_exists(f"seek_{key}"):
      ...
      try:
        ...
        if save_to_db:
          job_id = 'seek_' + key
          cover_letter = None
          if create_cover_letter and int(job_data_dict[key]['job_fit_evaluation']['Relevant Experience']['rating']) >= 4:
...
seek_jobs_dict = evaluate_seek_job_listings(search_term, number_of_jobs, job_searh_radius_km)